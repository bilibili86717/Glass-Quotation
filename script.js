...
const state = {
  step: 0,                  // 当前步骤索引
  totalSteps: PROCESSES.length + 2, // 0:客户信息 1:产品规格 2..N+1:17道工序 最后:结果
  ...
  // 3个独立开关
  exportProcessCost: true,      // 导出时显示工序成本明细
  exportProcessDetails: true,   // 导出时显示工序详细参数
  exportCostDetail: true,       // 导出时显示总成本/税金/利润
  totalCost: 0
};
    ...
    <div class="step-card">
      <div class="step-header">
        ...
        <div>
          ...
          <div class="step-subtitle">${p.fields.length} 个参数 · 不填则跳过此工序</div>
        </div>
      </div>
...
function renderResultPage() {
  ...
  return `
    <div class="step-card" style="box-shadow:none; padding:0 0 20px; max-width:760px;">
      ...
      ${state.exportProcessCost ? `<div class="cost-breakdown">${detailHtml}</div>` : ''}

      <div class="summary-card">
        ${state.exportCostDetail ? `
      ...
      <div class="toggle-row">
        <div class="toggle-label">📋 ${state.exportProcessCost ? '工序成本明细（开）' : '工序成本明细（关）'}</div>
        <div class="toggle-switch ${state.exportProcessCost ? 'on' : ''}" id="toggle-process-cost"></div>
      </div>

      <div class="toggle-row">
        <div class="toggle-label">📐 ${state.exportProcessDetails ? '工序详细参数（开）' : '工序详细参数（关）'}</div>
        <div class="toggle-switch ${state.exportProcessDetails ? 'on' : ''}" id="toggle-process-details"></div>
      </div>

      <div class="toggle-row">
        <div class="toggle-label">💰 ${state.exportCostDetail ? '总成本/税金/利润（开）' : '总成本/税金/利润（关）'}</div>
        <div class="toggle-switch ${state.exportCostDetail ? 'on' : ''}" id="toggle-cost-detail"></div>
      </div>
...
function bindInputs() {
  ...
  if (s === total - 1) {
    // 结果页 - 3个独立开关
    document.getElementById('toggle-process-cost').addEventListener('click', () => {
      state.exportProcessCost = !state.exportProcessCost;
      render();
      showToast(state.exportProcessCost ? '📋 导出时将显示工序成本' : '📋 导出时将隐藏工序成本', '');
    });
    document.getElementById('toggle-process-details').addEventListener('click', () => {
      state.exportProcessDetails = !state.exportProcessDetails;
      render();
      showToast(state.exportProcessDetails ? '📐 导出时将显示详细参数' : '📐 导出时将隐藏详细参数', '');
    });
    document.getElementById('toggle-cost-detail').addEventListener('click', () => {
      state.exportCostDetail = !state.exportCostDetail;
      render();
      showToast(state.exportCostDetail ? '💰 导出时将显示成本/税金/利润' : '💰 导出时将隐藏成本/税金/利润', '');
    });
  ...
  p.fields.forEach(f => {
    ...
    input.addEventListener('input', () => {
      ...
      state.processes[p.id].fields[f.id] = val;
      // 如果任意参数>0，则启用此工序
      const hasValue = p.fields.some(ff => (parseFloat(state.processes[p.id].fields[ff.id]) || 0) > 0);
      state.processes[p.id].enabled = hasValue;
      if (hasValue) calcProcessCost(p);
...
function buildReportHTML(forWord = false) {
  ...
  // 工序成本明细
  let processCostHtml = '';
  if (state.exportProcessCost) {
    let rows = '';
    PROCESSES.forEach(p => {
      const r = state.results[p.id];
      const enabled = state.processes[p.id].enabled;
      if (enabled && r) {
        rows += `<tr><td>${p.icon} ${p.name}</td><td class="cost">${fmt(r.cost)}</td></tr>`;
      }
    });
    if (rows) {
      processCostHtml = `<h2>工序成本明细</h2><table><tr><th style="width:60%;">工序名称</th><th style="width:40%;text-align:right;">每块成本（元）</th></tr>${rows}</table>`;
    }
  }

  // 工序详细参数
  let processDetailHtml = '';
  if (state.exportProcessDetails) {
    let details = '';
    PROCESSES.forEach(p => {
      if (!state.processes[p.id].enabled) return;
      let rows = '';
      p.fields.forEach(f => {
        rows += `<tr><td style="background:#f9fafb;">${f.label}</td><td class="cost">${state.processes[p.id].fields[f.id]} ${f.unit || ''}</td></tr>`;
      });
      const r = state.results[p.id];
      if (r && r.detail) {
        Object.entries(r.detail).forEach(([k, v]) => {
          rows += `<tr><td style="background:#fef9c3;">${k}</td><td class="cost">${typeof v === 'number' ? fmt(v) : v}</td></tr>`;
        });
      }
      details += `<h3>${p.icon} ${p.name}</h3><table>${rows}</table>`;
    });
    if (details) {
      processDetailHtml = `<h2>工序详细参数</h2>${details}`;
    }
  }

  // 成本汇总
  let costSummaryHtml = '';
  if (state.exportCostDetail) {
    costSummaryHtml = `
      <tr><td>总成本</td><td class="cost">${yuan(state.totalCost)}</td></tr>
      <tr><td>税金（${Math.round(CONFIG.taxRate * 100)}%）</td><td class="cost">${yuan(tax)}</td></tr>
      <tr><td>利润（${Math.round(CONFIG.profitRate * 100)}%）</td><td class="cost">${yuan(profit)}</td></tr>
    `;
  }
  ...
  let html = `
    ...
    <table>
      ${costSummaryHtml}
      <tr class="recommend-row"><td>✨ 推荐报价（每块）</td><td class="cost">${yuan(recommended)}</td></tr>
      <tr class="total-row"><td>📦 整批总价（× ${state.product.quantity} 块）</td><td class="cost">${yuan(recommended * state.product.quantity)}</td></tr>
    </table>
    ${processDetailHtml}
    <p style="margin-top:30px; color:#9ca3af; font-size:10pt; text-align:center;">本报价由玻璃成本报价系统生成 · 仅供参考 · 最终成交价以合同为准</p>
    </body></html>`;
  return html;
}
